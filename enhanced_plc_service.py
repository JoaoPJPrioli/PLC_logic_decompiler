...
class EnhancedPLCProcessingService:
    ...
    def _generate_output_files(self, results: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            # 3. Variable Summary (CSV)
            try:
                import csv
                variables_file = self.output_dir / f"variables_summary_{timestamp}.csv"
                ...
                with open(variables_file, 'w', newline='', encoding='utf-8') as f:
                    writer = csv.writer(f)
                    writer.writerow(['Name', 'Data Type', 'Scope', 'Description'])
                    
                    # Controller variables
                    for var in enhanced_vars.get('controller_scope', []):
                        writer.writerow([var.get('name'), var.get('data_type'), var.get('scope'), var.get('description')])
                ...
                logger.warning("CSV module not available for variables summary")
